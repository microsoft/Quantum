...
# This python script implements Remez' algorithm.
...
#
# Alternatively, `runRemez` can be imported and run directly, see the docstring
# of runRemez.
...
def _getErrors(exactvals, polycoeff, nodes):
...
"""
def runRemez(fun, a, b, d=5, odd=False, even=False, tol=1.e-13):
    finished = False
    # initial set of points for the interpolation
    cn = getChebyshevNodes(d + 2, a, b)
    # mesh on which we'll evaluate the error
    cn2 = getChebyshevNodes(100 * d, a, b)
    ...
    while not finished and len(cn)==d+2 and it < 50:
        ...
        errs = getErrors([fun(c) for c in cn2], sccoeff, cn2)
        maximum_indices = []
...
if __name__ == "__main__":
    ...
    # run Remez' algorithm
    err, coeffs = runRemez(f, a, b, degree, odd, even)
