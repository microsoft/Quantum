...
def guess_quantum(perm, index):
	"""The quantum estimation which uses the quantum algorithm in the Q# file.
	...
	if result == 0:
		...
		# on the right-hand side, in the original reference paper.
		if guess <= 0.5505:
			return 1
...
def guess_order(perm, index, n):
	...
	# This object counts the number of times the quantum algorithm guesses a given order.(so { order: count })
	q_guesses = { k+1 :0 for k in perm}
	# This object counts the number of times the classical algorithm guesses a given order.(so { order: count })
	...
	for order, count in c_guesses.items():
		# Return the percentage of each order guess, which = (num_of_guesses / total_guesses) * 100.
		print(f"{order}: {round((count / n)*100, 2)}%")
