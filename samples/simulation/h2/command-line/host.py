...
# Call a Q# operation that takes unit `()` as its input, and returns the static bond lengths[float].
...
for i in range(len(bond_lengths)):
	...
	est = min([H2Simulation.H2EstimateEnergyRPE.simulate(idxBondLength=i, nBitsPrecision=6, trotterStepSize=1.0) for _ in range(3)])
	print(f"\tEst: {est}\n")